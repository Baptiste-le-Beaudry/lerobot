...
class LeKiwiClient(Robot):
    ...
    def _poll_and_get_latest_message(self) -> str | None:
        ...
        try:
            socks = dict(poller.poll(self.polling_timeout_ms))  # timeout en ms
        except zmq.ZMQError as e:
            logging.error(f"ZMQ polling error: {e}")
            return None

        if self.zmq_observation_socket not in socks or socks[self.zmq_observation_socket] != zmq.POLLIN:
            # Pas de donnée prête dans le timeout
            return None

        # Maintenant on récupère le message (sans timeout, car on sait que c’est prêt)
        try:
            msg = self.zmq_observation_socket.recv_string(flags=zmq.NOBLOCK)
        except zmq.Again:
            # Pas de message dispo (rare ici, car poll indique prêt)
            return None
