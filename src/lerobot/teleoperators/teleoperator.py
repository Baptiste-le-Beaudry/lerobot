...
class Teleoperator(abc.ABC):
    ...
    def __init__(self, config: TeleoperatorConfig):
        ...
        print("Calibration loaded from", self.calibration_dir)
    """
    from pathlib import Path
    # Set these in ALL subclasses
    ...
    def __init__(self, config: TeleoperatorConfig):
        # 1. Convertir en Path
        cfg_path = Path(config.calibration_dir) if config.calibration_dir else None
        self.id = config.id
        # 2. Si cfg_path est un fichier, le dossier parent est calibration_dir
        if cfg_path and cfg_path.is_file():
            self.calibration_dir = cfg_path.parent
        else:
            # Sinon, c'est un dossier ou None → on prend le dossier par défaut
            base = HF_LEROBOT_CALIBRATION / TELEOPERATORS / self.name
            self.calibration_dir = cfg_path or base

        # Créer le dossier s’il n’existe pas
        self.calibration_dir.mkdir(parents=True, exist_ok=True)

        # 3. Construire le chemin complet du fichier
        ...
        # 5. Logger le chemin du fichier (et non juste le dossier)
        print("Calibration file used:", self.calibration_fpath)
