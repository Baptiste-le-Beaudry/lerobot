...
EPISODE_TIME_SEC = 40
RESET_TIME_SEC = 10
TASK_DESCRIPTION = "go_to_blue_lego" # Description of the task to be recorded change it as needed
...
parser.add_argument(
    ...
    type=str,
    required=True,
    help="ID du dépôt à utiliser pour le dataset (ex: Baptiste-le-Beaudry/go_to_lego_test19)"
)
...
if args.resume:
    dataset = LeRobotDataset(
        repo_id=repo_id,
        root=None,   # dossier où cloner/réutiliser
    )
...
while recorded_episodes < NUM_EPISODES and not events["stop_recording"]:
    try:
        log_say("🚀 Démarrage de l'enregistrement...")
        log_say(f"Recording episode {recorded_episodes}")
        if  robot.is_connected != True:
            robot.connect()
            leader_arm.connect()
            keyboard.connect()
